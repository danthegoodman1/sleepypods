...
| Complete | Crash/restart tests cover every lock boundary. | Durable active-row restart behavior is covered in real Postgres and through a real control-plane pod restart in kind while a stateful owner materialization holds the key. `postgres_store::crash_boundaries` injects process loss at each boundary against real PostgreSQL by claiming a reconciliation lease and expiring it, which is how a row looks once its owner is gone: after key acquire before apply, after apply before ready with the owner's object refs recorded, and during cleanup after an accepted deletion. Each case proves a same-key contender still conflicts across the crash and a fresh owner claims the work as a later attempt. |
